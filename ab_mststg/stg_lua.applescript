...
tell application "iTerm"
    ...
    tell first session of current tab of current window
        write text "ssh -t -t bot_jump sudo ssh stglua1"
    end tell

    -- Bottom Left tab (log 1)
    tell second session of current tab of current window
        write text "ssh -t -t bot_jump sudo ssh stglua1 tail -f /tmp/lua_error.log"
    end tell

    -- Top Right tab (editing window 2)
    tell third session of current tab of current window
        write text "ssh -t -t bot_jump sudo ssh stglua2"
    end tell

    -- Bottom Right tab (log 2)
    tell fourth session of current tab of current window
        write text "ssh -t -t bot_jump sudo ssh stglua2 tail -f /tmp/lua_error.log"
    end tell
end tell
